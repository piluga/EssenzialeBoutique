...
function calcolaPuntiSpesa(bonusApplicato = 0) {
    ...
    let regole = regoleSalvate ? JSON.parse(regoleSalvate) : { "PROFUMERIA": 1, "COSMETICA": 1, "DEFAULT": 0.25 };

    carrello.forEach(item => {
        // --- NOVITÀ: GERARCHIA PUNTI (Punto specifico vince su Categoria) ---
        if (item.punti_fedelta && item.punti_fedelta > 0) {
            puntiGuadagnati += (item.punti_fedelta * item.qta);
        } else {
...
window.confermaVendita = async function (riscattaBonus) {
    ...
    // 🔥 SCARICO GIACENZE E ESPLOSIONE KIT DINAMICI AL CHECKOUT
    await scaricaGiacenzeMagazzino(carrello);
...
window.magNuovoProdotto = function () {
    ...
    document.getElementById('mag-categoria').value = 'PROFUMERIA';
